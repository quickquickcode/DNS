...
def convert_small_topo_correct():
    ...
    IMAGE_MAP = {
        ...
        "gzdx_dns_client_0.7": "992a8200-7b3a-4466-a2f0-2ad559f40802",
        "gzdx_dns_client_0.8": "992a8200-7b3a-4466-a2f0-2ad559f40802",  # 0.8版本映射到相同镜像ID
    ...
    FLAVOR_MAP = {
        ...
        "gzdx_dns_client_0.8": "RS00-001-004096-00040"  # 0.8版本使用相同配置规格
    }
    ...
    for i, node in enumerate(src['nodes']):
        ...
        for service_ip, service_info in node['type'].items():
            ...
            if image_name == "gzdx_dns_client_0.7":
                image_name = "gzdx_dns_client_0.8"
    ...
    for node in src['nodes']:
        ...
        ip_parts = base_ip.split('.')
        mgmt_gateway_ip = f"{ip_parts[0]}.{ip_parts[1]}.{ip_parts[2]}.1"
    ...
    for node in src['nodes']:
        ...
        for service_ip, service_info in node['type'].items():
            ...
            # 镜像版本重新映射：gzdx_dns_client_0.7 -> gzdx_dns_client_0.8
            if image_name == "gzdx_dns_client_0.7":
                image_name = "gzdx_dns_client_0.8"
